...
def _oauth():
    ...
    d = os.path.dirname(config.SPOTIFY_TOKEN_CACHE)
    if d:
        os.makedirs(d, exist_ok=True)
    return SpotifyOAuth(
        client_id=config.SPOTIFY_CLIENT_ID, client_secret=config.SPOTIFY_CLIENT_SECRET,
        redirect_uri=config.SPOTIFY_REDIRECT_URI, scope=_SCOPE,
        cache_path=config.SPOTIFY_TOKEN_CACHE, open_browser=False,
    )
...
def create_playlist(name: str, tracks: list) -> dict:
    ...
    pl = sp.user_playlist_create(uid, name, public=config.SPOTIFY_PLAYLIST_PUBLIC,
...
def reset() -> None:
    """Clear cached playlist lookups so new credentials/playlists take effect."""
    _pl_cache.clear()
    _pl_tracks.clear()
